...
size_t runcodestack_max;
VAR(debugruncodestack, 0, 0, 1);
VAR(runcodestack, 1, 0, 0);
...
static const uint *runcode(const uint *code, tagval &result)
{
    ...
    int numargs = 0;
    if(debugruncodestack && !rundepth) runcodestack_max = (size_t)&numargs;
...
exit:
    ...
    --rundepth;
     ...
     {
        size_t runcodestack_min = (size_t)&runcodestack_min;
        size_t runcodestack_kB = (runcodestack_max - runcodestack_min) / 1024;
